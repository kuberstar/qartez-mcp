...
fn qartez_arch_review_prompt_biases_map_on_focus_keyword() {
    ...
    let text_untargeted = user_text(&untargeted);
    assert!(text_untargeted.contains("qartez_map"));
    assert!(text_untargeted.contains("qartez_stats"));
    assert!(text_untargeted.contains("qartez_security"));
    assert!(text_untargeted.contains("qartez_hotspots"));
    // boundaries must be preceded by a qartez_wiki call to populate clusters.
    assert!(text_untargeted.contains("qartez_wiki"));
    ...
    assert!(
        targeted
            .description
            .as_deref()
            .is_some_and(|d| d.contains("auth"))
    );
}

#[test]
fn qartez_arch_review_prompt_chains_wiki_before_boundaries() {
    let (server, _dir) = make_server();
    let result = server.qartez_arch_review_prompt(Parameters(SoulArchReviewArgs { focus: None }));
    let text = user_text(&result);
    let wiki = text.find("qartez_wiki").expect("wiki step present");
    let boundaries = text
        .find("qartez_boundaries")
        .expect("boundaries step present");
    assert!(
        wiki < boundaries,
        "qartez_wiki must be invoked before qartez_boundaries so the cluster table exists"
    );
}

...
- **Cross-file dependency graph for infrastructure repos (Kustomize / Helm / ArgoCD / Terraform)** - a new structured-config parser (`index/languages/yaml.rs`) extracts import edges from `kustomization.yaml` resources/bases/components/patches, Helm `Chart.yaml` `dependencies`, and ArgoCD `Application` source paths, and a companion resolver in `index/mod.rs` binds infra path references (bare siblings, directory -> `kustomization.yaml`/`Chart.yaml` entrypoints, Terraform `module` dir -> `.tf`) into the edges table. This lights up every graph-tier tool (`qartez_deps`, `qartez_impact`, `qartez_diff_impact`, `qartez_hotspots`, `qartez_health`) on GitOps/IaC trees that previously indexed as isolated objects. Validated on a real `kuberstar-infra` `k8s-apps` tree: 0 -> 267 dependency edges. Path normalisation is lexical (`..` cannot escape the root) and Windows-safe.
- **`qartez_path` tool - shortest call/reference path between two symbols** - a new analysis-tier tool that runs a forward BFS over the persisted `symbol_refs` graph from a `from` symbol to a `to` symbol and returns the ordered symbol/file chain plus a count of alternative shortest paths. The default graph combines `call` and `type` edges; the optional `kind` parameter restricts traversal to a single edge kind. Same-symbol, no-path, and ambiguous-endpoint cases are reported gracefully. Covered by unit tests (path found, no-path, same-symbol, missing-symbol, kind-filter).
- **`qartez_unused` whole-program reachability mode** - an opt-in `reachable=true` parameter computes dead code by forward-walking the reference graph from real roots (`main`, exported public API, framework entry-points, tests) instead of the default one-hop importer check, catching functions reachable only from other dead code. Default behaviour is unchanged.
- **`qartez_security` source -> sink taint reachability** - an opt-in `taint_reachability=true` parameter runs a second pass that annotates dangerous sink findings (SQL / command exec / deserialization) with whether they are call-graph reachable from a user-input source, plus the connecting path. Coarse reference-level reachability, not variable taint; annotation-only, so it never suppresses or reorders findings. Default off.

### Changed

- **Startup now reconciles the tree on every launch** - a matching workspace fingerprint previously skipped the index walk entirely at MCP startup, so files added/modified/deleted while the server was down (relying on the live-only watcher) were never indexed. The cheap reconciliation walk now always runs; only the expensive global recompute (PageRank, co-change) stays gated on a fingerprint or file-set change, via a new `IndexOutcome { updated, deleted }` return. Fast-startup latency is preserved by skipping the derived-table rebuilds (`sync_fts`, unused-exports materialisation) when the walk changed nothing.
- **Migrated off the unmaintained `serde_yaml`** - all YAML handling now uses `serde_yaml_ng`; no residual `serde_yaml` references remain.
- **Documented tool count reconciled to 43** - `README`, `mcp_instructions`, `docs/tools`, `docs/agent-guide`, and `docs/architecture` match the `total_tool_count_is_43` invariant after the `qartez_path` addition.
- **Dependency refresh** - `git2` 0.20.4 -> 0.21.0 (adapts to the 0.21 API where `final_signature()` and `Signature::name`/`summary` return `Option`/`Result`; null signatures are handled as unknown author) and `anyhow` 1.0.102 -> 1.0.103 from the advisory patches above, plus a release-time lockfile regeneration picking up `rmcp` 1.7.0 -> 1.8.0 (the MCP protocol framework), `openssl` 0.10.80 -> 0.10.81, `tree-sitter` 0.26.9 -> 0.26.10, and routine patch bumps across the transitive tree. The full suite (build, test, clippy `--all-features`, `cargo-deny check advisories bans licenses sources`) was re-verified green on the regenerated lock; no qartez API changes.

### Fixed

- **Startup aborted on read-only project trees** - a read-only mount (Google read-only source sandbox, Nix store, container image layers) made `.qartez/index.db` creation fail before the MCP `initialize` handshake, surfacing to clients as `Read-only file system (os error 30) ... "initialize": EOF`. `resolve_db_path` now write-probes the in-tree `.qartez/` location and, when it is not writable, stores the index under a per-project cache dir (`XDG_CACHE_HOME` / `~/.cache`, then the OS temp dir) keyed by a hash of the project path. An explicit `--db-path` is still honoured verbatim.
- **`qartez-setup` aborted under external MCP sync** - MegaClaude/MegaCoder mirror qartez into `~/.codex/config.toml` inside their own sync-marker blocks; setup saw that as an unmanaged `[mcp_servers.qartez]` section and bailed, and manual removal would have produced a duplicate TOML key on the next sync. `install_codex` now detects a qartez entry inside any external MCP-sync block and returns success without writing, since qartez is already reachable from Codex.
- **Infrastructure dependency-graph hardening** - a Helm `values.yaml` is no longer misclassified as a Kustomization (the `resources:` requests/limits mapping is distinguished from a kustomize `resources:` sequence); ArgoCD raw-manifest directories no longer link the entire tree from a root-relative miss; multi-root/submodule mode resolves import edges within the correct root instead of a sibling root whose prefix collides with a subdirectory name, and within-root refs are prefixed consistently.
- **Symbol-mutator correctness (from the `/boost` pass)** - `qartez_replace_symbol` no longer rejects braced-initializer bodies; `qartez_replace_symbol` / `qartez_insert_*` / `qartez_safe_delete` preserve CRLF line endings on uniformly-CRLF files; an unsound `std::env::set_var` on the multi-threaded tokio runtime was replaced with an `AtomicBool` (`set_defer_compaction`); and the incremental pure-delete path and the `qartez_hotspots` / `qartez_health` per-file complexity queries were de-N+1'd.
- **RustSec advisories patched** - `anyhow` 1.0.103 (RUSTSEC-2026-0190, `Error::downcast_mut` unsoundness) and `git2` 0.21.0 (RUSTSEC-2026-0183 / RUSTSEC-2026-0184, null-pointer UB).
